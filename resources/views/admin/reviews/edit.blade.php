        ...
        <form action="{{ route('admin.reviews.update', $review->id) }}" method="POST">
            ...
            <select name="rating" id="rating" class="form-select" required>
              ...
              @for ($i = 1; $i <= 5; $i++)
                <option value="{{ $i }}" {{ $review->rating == $i ? 'selected' : '' }}>
                  {{ $i }} ⭐ {{ $i == 1 ? '(Sangat Buruk)' : ($i == 2 ? '(Buruk)' : ($i == 3 ? '(Cukup)' : ($i == 4 ? '(Baik)' : '(Sangat Baik)'))) }}
                </option>
              @endfor
            </select>
          </div>
          ...
          <div class="mb-3">
            ...
            <div class="d-flex gap-2">
              <span class="badge
                ...
                @elseif($review->status === 'approved') Disetujui
                @else Ditolak
                @endif
              </span>
            </div>
          ...
          <div class="mb-3">
            ...
            <div class="row g-2">
              ...
              <div class="col-auto">
                <img src="{{ asset('storage/' . $photo) }}" alt="Review photo" class="rounded" style="width: 100px; height: 100px; object-fit: cover;">
              </div>
              @endforeach
            </div>
          </div>
